...
object NotificationHook : HookLifecycleListener {
    ...
    private fun initWithDexKit(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        if (notifyClass != null) {
            ...
            cacheWarmedUp = true
            LogUtil.i(TAG, "DexKit found: ${notifyClass.name}")
        }
    }
    ...
    private fun filterNotifications(param: XC_MethodHook.MethodHookParam) {
        try {
            val blacklist = WxIdBlacklist.getWxIds()
            if (blacklist.isEmpty()) return

            HookMetrics.recordHookExecution(TAG)
            LogUtil.d(TAG, "Filtering notifications, blacklist size: ${blacklist.size}")
        } catch (e: Throwable) {
            LogUtil.e(TAG, "Error filtering notifications: ${e.message}", e)
        }
    }
}